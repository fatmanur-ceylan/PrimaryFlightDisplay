...
class PrimaryFlightDisplay(WidgetClass):
    ...
    def keyPressEvent(self,e):
        if e.key() == Qt.Key_Up:
            self.airspeed = self.airspeed+50
            print("Hız artırıldı",self.airspeed)
        if e.key() == Qt.Key_Down:
            self.airspeed = self.airspeed -50 
            if self.airspeed <0 :
                self.airspeed = 0
                print("Hız Sıfırda!!")
            print("Hız Azaltıldı",self.airspeed)

        if e.key() == Qt.Key_W:
            self.alt = self.alt + 10
            print("İrtifa arttırıldı",self.alt)
            self.pitch = self.pitch + 5
        if e.key() == Qt.Key_S:
            self.alt = self.alt - 10
            print("İrtifa alçaltıldı",self.alt)
            self.pitch = self.pitch - 5

        if e.key() == Qt.Key_A:
            self.vspeed = self.vspeed + 1 
            print("Vertical Speed arttırıldı!")
        if e.key() == Qt.Key_D:
            self.vspeed = self.vspeed - 1 
            print("Vertical Speed Azaltıldı!")
    ...
    def draw_altimeter(self):
        ...
        l1 = int(((y1 + y2) / 2 + self.alt * r - y2) / per) - 1
        l2 = int(((y1 + y2) / 2 + self.alt * r - y1) / per) + 1
        ...
        for i in range(l1, l2 + 1):
            ...
            for j in range(-5, 6):
                painter.drawLine(QLineF(x1, (y1 + y2) / 2 + pos + j / 5 * per, x1 + 8 * s, (y1 + y2) / 2 + pos + j / 5 * per))
    ...
    def draw_airspeed(self):
        ...
        painter.save()
        painter.setClipRect(QRectF(x1, y1, x2 - x1, y2 - y1))
        l1 = int(((y1 + y2) / 2 + self.airspeed * r - y2) / per) - 1
        l2 = int(((y1 + y2) / 2 + self.airspeed * r - y1) / per) + 1
        for i in range(l1, l2 + 1):
            pos = -i * per + self.airspeed * r
            if (i % 2) == 0:
                painter.drawLine(
